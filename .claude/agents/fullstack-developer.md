...
1. **Preserve each page's write model.** Existing DailyPlan, XP, and config whole snapshots remain unmerged except for DailyPlan's documented direction-only and clear-ticks writes. `exerciseDays` intentionally uses targeted nested updates so editing one exercise cannot erase another.
