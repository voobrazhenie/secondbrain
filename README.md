...
- `dailyplan/` is the separate recurring-task checklist. Its routine — the categories, their items, and the date day one is counted from — belongs to whoever is signed in, in `users/{uid}/config/plan`. There is no routine in this repository: a new account opens with an empty list and fills it in. The page edits that routine: a dashed slot under the list adds a category, a long press on a category header renames or removes one, and a REORDER button puts a drag grip on every row and header so tasks can be moved within a category or into another one. Removing a category leaves its tasks in a nameless group at the top of the day. Streaks are a separate list in `users/{uid}/config/streaks` — each points at a card in the day and counts either the days in a row it was ticked or the days since it last was, for whichever date is on screen. A "days in a row" streak keeps its run and its last ticked day on the record and draws from those, because the day history it would otherwise be counted from arrives after the first paint; the history is what checks the stored run rather than what the number depends on.
...
- `pomodoro/` is the desk timer: thirty minutes of work, five minutes on your feet. Two buttons start a round and nothing starts itself — a round that runs out chimes, says so, and waits. The running round is stored as an end time in `users/{uid}/config/pomodoro`, not as a number counting down, so backgrounding the app or reloading the page picks it up where it was.
